...
function loadProj(jsnName){
  ...
  $.post( "/story/delete", function( data ) {
    console.log("---------------------------------------------------------");
    console.log(data);
    console.log("---------------------------------------------------------");
  });
...
function releases(ReleaseId) {
  ...
  $.getJSON( "/release?id="+ReleaseId+"", function( data ) {
    ...
    items.push("<h3 class = 'pull-right' id=\"chartBtn\"><i class=\"fa fa-bar-chart\"data-toggle=\"modal\" data-target=\"#graphModal\" onclick=\"drawGraph('storyStatus',false);\"></i></h3>");
    $( "<div/>", {
      "class": "headingBox",
      html:  items.join( "" )
    }).appendTo( ".release" );
